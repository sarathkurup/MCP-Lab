...
purpose so every Workbench feature has something real to find.
...
2. The three servers appear in the **MCP Workbench** view — they are declared in
   `mcp.config.json`, so add them via **MCP: Add Server** or copy them into
   `mcpWorkbench.servers` in settings.
