...
const TOKEN_KEY = 'mcpWorkbench.bridgeToken';
...
export class WorkbenchMcpServer implements vscode.Disposable {
  ...
  constructor(private readonly workbench: Workbench) {
    this.role = new McpServerRole({
      ...
      version: '0.1.0',
      instructions: [
        'MCP Workbench aggregates the MCP servers a developer has configured.',
  ...
  async start(): Promise<{ url: string; token: string }> {
    ...
    this.workbench.logs.log('info', `Workbench MCP server listening on ${this.endpoint}`);
    return { url: this.endpoint!, token: this.token };
  }
  ...
  async stop(): Promise<void> {
    ...
    this.workbench.logs.log('info', 'Workbench MCP server stopped');
  }
  ...
  private registerTools(): void {
    this.role.register({
      tool: {
        name: 'listMcpServers',
        description:
          'Lists the MCP servers configured in MCP Workbench, with their status, health and what they expose.',
    ...
    this.role.register({
      tool: {
        name: 'getMcpLogs',
        description: 'Returns recent Workbench and server log lines, useful when a call failed.',
        inputSchema: {
          type: 'object',
          properties: {
            server: { type: 'string' },
    ...
    this.role.register({
      tool: {
        ...
        description: 'Runs Workbench diagnostics against a server: connectivity, protocol, schema quality, security, coverage.',
        inputSchema: {
          type: 'object',
  ...
  private requireConnection(name: string) {
    ...
    if (connection.status !== 'connected') {
      throw new Error(
        `"${connection.config.name}" is ${connection.status}. Ask the developer to connect it in MCP Workbench.`,
      );
    }
    return connection;
  }
