...
function readme(options: ScaffoldOptions, run: string[]): string {
  ...
  return [
    ...
    '## Working on it',
    '',
    'This project ships an `mcp.config.json`, so MCP Lab and the',
    '`mcplab` CLI can both reach it:',
    '',
    '```bash',
    'mcplab doctor --config mcp.config.json',
    'mcplab lint   --config mcp.config.json',
    'mcplab test   --config mcp.config.json',
    '```',
    '',
