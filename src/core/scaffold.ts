...
function readme(options: ScaffoldOptions, run: string[]): string {
  ...
  return [
    ...
    '## Working on it',
    '',
    'This project ships an `mcp.config.json`, so MCPilot and the',
    '`mcpilot` CLI can both reach it:',
    '',
    '```bash',
    'mcpilot doctor --config mcp.config.json',
    'mcpilot lint   --config mcp.config.json',
    'mcpilot test   --config mcp.config.json',
    '```',
    '',
