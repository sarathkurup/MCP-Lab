...
export async function diagnose(
  ...
  checks.push({
    ...
    detail: caps.logging
      ...
      : 'No logging capability; only stderr and MCP Lab events are available',
  });
  ...
  if (probe && caps.tools) {
    try {
      await client.callTool('__mcplab_probe__', {});
      checks.push({
        id: 'errors',
  ...
  if (connection.config.transport === 'http') {
    checks.push({
      ...
      fixCommand: options.hasCredential ? undefined : 'mcplab.setAuthToken',
    });
  ...
  if (options.testedTargets) {
    ...
    checks.push({
      ...
      fixCommand: 'mcplab.generateTests',
    });
  } else {
    checks.push({
      id: 'coverage',
